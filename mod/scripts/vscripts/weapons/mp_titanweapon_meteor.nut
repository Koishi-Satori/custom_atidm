...
{
        if( weapon.HasMod( "tcp_shotgun" ) )
                return OnWeaponPrimaryAttack_FireWallShotGun( weapon, attackParams, true )
...
{
        if( weapon.HasMod( "tcp_shotgun" ) )
                return OnWeaponPrimaryAttack_FireWallShotGun( weapon, attackParams, false )
        weapon.EmitWeaponNpcSound( LOUD_WEAPON_AI_SOUND_RADIUS_MP, 0.2 )
...
var function OnWeaponPrimaryAttack_FireWallShotGun( entity weapon, WeaponPrimaryAttackParams attackParams, bool playerFired )
{
        float leftTime = 2.5
        ...
        {
                entity bolt = weapon.FireWeaponBolt( attackParams.pos, vel, 1, METEOR_DAMAGE_FLAGS, METEOR_DAMAGE_FLAGS, playerFired , 0 )
                if ( bolt )
                {
                        EmitSoundOnEntity( bolt, "weapon_thermitelauncher_projectile_3p" )
...
void function OnProjectileCollision_FireWallShotGun( entity projectile )
{
        for( int i = 3; i > 0; i-- )
        {
                ...
                vector up = AnglesToUp( trailAngles )
                vector v = velocity + forward * fireSpeed + up * fireSpeed
                entity prop = FireWallShotGun_CreatePhysicsThermiteTrail( origin, owner, inflictor, projectile, v, RandomFloatRange( thermiteLifetimeMin, thermiteLifetimeMax ), METEOR_FX_TRAIL, eDamageSourceId.mp_titanweapon_firewall_shotgun )
...
void function FireWallShotGun_PROTO_PhysicsThermiteCausesDamage( entity trail, entity inflictor, int damageSourceId, float damagePilot, float damageTitan )
{
        entity owner = trail.GetOwner()
        Assert( IsValid( owner ) )
        ...
        float METEOR_DAMAGE_TICK_PILOT = damagePilot
        float METEOR_DAMAGE_TICK = damageTitan
